...
namespace ServiceBus.Console2
{
    class Program
    {
        ...
        {
            var endpointConfiguration = new EndpointConfiguration("Console 1");
            ...
            while (true)
            {
                ...
                switch (key.Key)
                {
                    case ConsoleKey.Enter:
                        ...
                        logger.Info($"Sending message to Console 2: {command.Message}");

                        await endpointInstance.Send("Console 2", command)
                                              .ConfigureAwait(false);
